  ...
  Goals delivered: {goals_completed_count} this window{, of which ~{routine_count}
   ...
   routine_ratio × goals_completed_count when loop_state is present, else omit the clause}.
    ...
    {GRACEFUL OMIT (a FALLBACK -- no longer the expected default): IF NO recurring
     ...
     ⚠ THOSE ROWS ARE A DATED 2026-08-22 CENSUS, CORRECT AS HISTORY, AND LIVE RATES
     HAVE MOVED. Do NOT rewrite them -- three occurrences of g-001-04 have now proposed
     exactly that, and the right repair was always an annotation beside them, which is
     this line. Re-measured 2026-09-09 (zeta, hostname cc-02, uname -r 6.8.0-138-generic):
