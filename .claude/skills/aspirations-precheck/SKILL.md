...
| 0.5b.4a | defer-citation-parity-check | medium | `bash core/scripts/defer-citation-parity-check.sh`. Report-only, no --apply. Reports the population defer-recheck skips: a defer_reason naming a dependency in a form its id-extractor cannot capture. Dispatched since g-115-10477; this ROW was missing until g-374-64, so COVERAGE undercounted it |
| 0.5b.4b | defer-scope-coverage | medium | `bash core/scripts/defer-scope-coverage.sh --output json` (`--output json` REQUIRED — the default is human text, which the battery classifies BLIND). Report-only, no --apply and none may be added: inferring a scope from prose and storing it would launder a guess into a fact (reclaim-routed-work r2). The only consumer of the population 0.5b.3 skips (`skipped_free_form`, ~76% of its eligible rows); finding is `total_keyable` — the defers a backfilled scope would make reachable — never `total_unkeyable`, which is the floor. Built 2026-08-09 (g-115-3939), call-site-less until g-374-64 |
