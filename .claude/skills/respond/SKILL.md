...
| Update framework | "update yourself" / "update your framework" / "pull the latest framework" / "pull fresh from <staging Mind>" / "adopt the newest release" | Invoke `/update-framework`. Never web-search, pip install, or hand-copy: the framework arrives over git from the staging repo (`pull-promotion.md`); the skill detects git-fed vs transplant and runs the right path. |
