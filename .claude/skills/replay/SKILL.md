...
Apply spaced repetition filter:
  ...
  looking like compliance. A `surprise` ceiling of 5-6 is routine IN THE POOL, so
  rule 2 (`surprise >= 7`) usually contributes nothing and rule 1's top tie group
  is most of the band. Read that ceiling as a POOL fact, never a corpus one
  (guard-6131): the corpus reaches surprise 10 — high-surprise records are
  replayed, encoded and drained OUT of the pool, which is what creates the ceiling.

  ⚠ DO NOT CHOOSE A SORT DIRECTION FROM A MEASURED rc-VS-CORRECTED GRADIENT — THE
  DIRECTION IS NOT STABLE ACROSS MEASUREMENTS. This block prescribed stratifying
  because "an rc-asc tiebreak selects the band's LEAST-enriched corner", measured
  inside `surprise == 6` as rc=0 50.0% / rc=1 70.4% / rc=2 93.9%. Re-measured
  2026-09-15 over the resolved+archived corpus, same band, the gradient REVERSES —
  rc=0 is the HIGHEST of the first three — and across all bands it is
  non-monotonic. THE PRESCRIPTION SURVIVES; ITS STATED REASON DOES NOT, and the
...
# the key the text came from, or NULL only when EVERY link is empty. ⚠ NULL IS NOT
# THE BARE TEST: the chain's LAST link is `rationale`, written at FORMATION, so a
# record with no outcome lesson returns its own premise — for a CORRECTED record,
...
**EVERY NUMBER THIS STEP EMITS IS UPWARD-BIASED BY CONSTRUCTION, THREE LAYERS DEEP**
(guard-2129 / guard-2144 / guard-2148). The batch is violation-first, so it is
unrepresentative of the POOL; the pool excludes `encoded_via_chronic` records — a
population that is 100% CORRECTED — so it is unrepresentative of the CORPUS. Re-measuring
a batch finding against the pool removes only the first layer. THE CORPUS is the deduped
union of `pipeline-read.sh --stage resolved` AND `--stage archived`; `--stage resolved`
alone is a survivorship filter (a few percent of the union, and records migrate OUT of it
as they age), and `--replay-candidates` is not the corpus either. **Name the population on
every rate you write down** — the same by-month split runs in OPPOSITE directions on the
pool and on the corpus, with no error to warn you.

A guardrail cannot outvote the instrument it guards: guard-2129 sits in the guardrail
store, and this paragraph is the instrument.

**The method, in the order it must run.** Each step below has either manufactured or
destroyed a finding here; skipping one yields a confident number with no meaning.

1. **Derive the marker from `title`.** NEVER from the resolution narrative
   (`outcome_detail` and its nine fallback keys) — that text is written AFTER and ABOUT
   the outcome, so the predicate is a linguistic proxy for the variable being measured,
   and **de-circularization does not catch it** (guard-4758). `rationale` and
   `measurement_channel` carry a verdict token on roughly one record in five; `title` on
   about one in a hundred. If a marker must use a contaminated field, strip verdict tokens
   or drop contaminated records from BOTH arms and report the reduced n.
2. **De-circularize** — exclude the batch from both arms before computing anything. Not
   only a small-n corrective: it has removed 70% of an effect at a comfortable n=18, where
   3 of 18 group members were batch rows and nothing about the group size warned of it. If
...
4. **Report the EXCEEDANCE PROBABILITY** — the fraction of permutations with
   ...
   whatever the p95 says. Print the MEDIAN alongside it: a marker below the median is not
   close, which is what stops the next cycle relitigating it.
5. **Run the controls in the same call.** The date-free `id[11:]` checksum — `id[11:]`,
   NOT `id`: record ids begin `YYYY-MM-DD_`, date is the strongest predictor of CORRECTED
   in this corpus, and ~96% of the full-id "floor" was those ten characters. Plus a
   size-matched common-word-in-title split. Then compare the marker against the date-only
   `id[:10]` control directly — one line, and faster to read than any permutation table.

⚠ **THE 10pp THRESHOLD IN ITEMS 1-2 BELOW IS *BELOW* THE SIZE-MATCHED FLOOR FOR EVERY
GROUP UNDER ~100, SO "OVER 10pp" IS NOT EVIDENCE OF ANYTHING.** It is conservative in one
direction only: under it is nothing; over it is UNDETERMINED until the exceedance
probability decides. Measured on two corpora at two base rates, agreeing: the floor is
~10pp at n=100, ~14pp at n=50, ~23pp at n=20, ~30pp at n<=10. A marker at n~50 clearing
10pp sits inside the noise. Never treat the threshold as a bar a marker can clear.

**Do not hand-roll the batch-vs-corpus comparison — invoke `/compare-batch-vs-corpus-rate`**
(forged; its SKILL.md already declares `Called by: /replay --sharp-wave`). It does NOT
supply the size-matched permutation floor this section requires, and treating its output
as that floor drops the control (g-115-6627). Build the floor yourself.

📎 **The dated evidence behind every claim above — per-box readings, the floor tables, the
superseded corrections and what falsified each — lives in
`core/config/replay-instrument-readings.md`. ADD NEW READINGS THERE, NEVER HERE.** This
file is INJECTED WHOLE and bounded at 65,536 B; when the series was extracted on
2026-09-15 it was already 60 B over that ceiling, which means the part arriving TRUNCATED
was the tail — Steps 4 through 6, including the mandatory Step 4.5 stamp read-back
contract. Two prior extractions relied on a prose instruction exactly like this one and
were appended to anyway; what holds the line is the size gate refusing the commit, so
treat a refusal as the correct verdict rather than something to override (guard-6482).
...
**Do not hand-roll this step. Run `bash core/scripts/replay-stamp-verify.sh <id> ...`**
(gap-145; contract-tested by `test_replay_stamp_verify_contract.py`). It does
both the whole-object write and the per-id read-back below, compares the stored
VALUES, and emits `stamped`/`verified` counts as JSON (`--dry-run` previews).
It does NOT read `encoded_via_chronic`, so apply the skip above to the ids you pass it.
The pseudocode below is its SPEC. The wrapper shipped 2026-08-25, but this step never
named it, so occurrence 135 (bravo, hostname cc-05, 2026-09-16) hand-rolled
the loop again, and the wrapper's own author wrote that copy.
