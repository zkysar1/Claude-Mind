...
SIGNALS_JSON='{
  ...
  "confirming_signal_count":        {int — OPTIONAL and PREFERRED over the fraction (g-115-9566). The confirming NUMERATOR itself, before you divide it by self_evolution_signals_count. It makes the `>= 2` bar an exact integer comparison instead of a float product whose verdict could turn on typed decimal precision; absent it the helper recovers the numerator from the fraction by half-up rounding. You computed the fraction from two counts, so emit the count too},
