...
temporal credit (8.9), and relative advantage (8.10) in one Python pass — pure
arithmetic, no LLM judgment. This is the ONLY path; any bug belongs in
`state-update-audit.py`, never in a reintroduced shadow LLM path here.

Steps 8.5 (Actionable Findings Gate), 8.75 (Execution Reflection), 8.76 (Skill
Quality) and 8.11 (Execution Feedback) stay on the LLM path — those need
judgment `run-all` does not cover.

The full spec — the four mandatory `compute_learning_value` flags (omitting any
one silently yields learning_value=0 via argparse defaults), the run-all
invocation, and every consumer step — lives in the digest. Load it ONLY when
this iteration actually reaches Steps 8.8-8.10:

```
Bash: load-state-update-audit-digest.sh → IF path returned: Read it
Follow the digest inline. It is the authoritative spec.
```

<!-- g-115-7044: 19,891 B extracted to core/config/state-update-audit-digest.md
     to bring this hot-path SKILL.md under the 65,536 B injection budget. Do NOT
     re-inline — reload cost is the point (cf. aspirations-loop-digest.md). -->
