...
| `inbound-drain` (**executable slot** — `world/scripts/inbound-drain.sh`, not a `.md`) | `core/scripts/inbound-drain-run.sh` (the audited entry point), dispatched every iteration by the always-run precheck battery as lane `inbound-drain` (phase 0.5b.1e). | The domain's answer to "what does draining a queued external instruction mean here" — which spool, which drain script, and **whether this box should drain at all**. The vessel guard is the load-bearing half: a box that mounts EVERY environment's spool (the operator) would otherwise drain many members' instructions into ONE box's queue — the drain working perfectly and doing something badly wrong. Contract: args pass through; ONE JSON object on stdout; exit 0 always. Emits `{"not_a_vessel":true,"reason":...}` when it declines, so absence is never reported as a count of zero. Run via `bash` (an own-cloud pull does not preserve `+x`). | none — no slot ⇒ the runner reports `no-slot` and the lane is a silent no-op, which is the supported fresh-world configuration. | On-demand — the domain writes `world/scripts/inbound-drain.sh` wrapping its own drain. Feedback path: the runner's `unparseable` status is a battery FINDING rather than a swallowed exit-0, so a slot whose output shape drifts from its consumer surfaces on the next iteration instead of reporting clean forever (g-369-150). |
