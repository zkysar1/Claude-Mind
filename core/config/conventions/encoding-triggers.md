...
| T25 | `/fresh-eyes-review` Phase 5.6 — non-routed observation encoding | ✓ | T + R + G | A, Au | Every 25 goals (cadence) OR user invocation — encodes briefing observations Phase 5.5 did NOT route to Self (`act_now`) or a goal (`act_later`); novelty-gated (edit-in-place vs new node) | `.claude/skills/fresh-eyes-review/SKILL.md` Phase 5.6 |
| T26 | `/fresh-eyes-program` Phase 5.6 — non-routed observation encoding | ✓ | T + R + G | A, Au | Every 100 goals (cadence) OR user invocation — encodes program-alignment observations not routed to a Program edit / goal by Phase 5.5; novelty-gated | `.claude/skills/fresh-eyes-program/SKILL.md` Phase 5.6 |
| T27 | `/fresh-eyes-tree` Phase 5.5 — taxonomy-finding encoding | ✓ | T + R + G | A, Au | Every 200 goals (cadence) OR user invocation — encodes taxonomy-health findings (no `act_now`/`act_later` path; all net-new); novelty-gated | `.claude/skills/fresh-eyes-tree/SKILL.md` Phase 5.5 |
...
| E18 | ✗ | The mind ELECTS to keep a perception (perception-bridge intake) | Every Txx/Exx trigger in this file fires on what the agent DID — a goal closed, a probe that surprised, a file edited, a user correction. None fires on what the mind PERCEIVED and judged worth keeping. The perception bridge delivers environment observations into the mind; the act of selecting one as encode-worthy is itself the signal, and it reaches no encoding lane. Distinct from E3 / E7, which key on CONTRADICTION with the tree — a perception can be novel, tree-consistent, and still worth encoding, and those two are structurally blind to exactly that case. | **OPEN.** Proposed: on a mind-side keep decision, append an `encoding_observation` to WM `sensory_buffer` carrying the perception's provenance (source environment, what was perceived, why kept), `replay_priority=routine_observations` unless it independently scores high-surprise — then the existing encoding gate does the category routing, as E14 does for curriculum promotion. No new store. Not yet scheduled: the plan states the bridge itself stays off, and the first Exx consumer cannot precede its producer. Source: tree node `perception-bridge-pearl` §18.4 (ZDS-Mind world; parent `perception-bridge-plan`), findings board `msg-20260911-230744-omni-378`; filed into this file by g-373-34. See that goal for the correction to the plan's stated reason the bridge stays off (guard-5514, upstream id — half that rationale is obsolete). |
