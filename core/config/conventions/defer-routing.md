...
| Defer-time write, routed to NOBODY (g-353-109) | `gates/defer_self_artifact.py`, Layer 0b of the daemon `update_goal`, on the FIELD (structured prefixes included). Refuses when the defer's own clause names the goal's OWN artifact (a branch or commit scope carrying its id, or a PR/sha its own record binds to one) AND says it has not landed. No Unblock is filed: the goal already owns the work. | The deadlock the human-routing gates cannot see: g-373-12 waited on its own open, clean PR #528 as an "external signal". Same `--force-defer`. Also ADVISES when a defer names a standing-grant action (merge/push/deploy/release) and cites no probe. |
