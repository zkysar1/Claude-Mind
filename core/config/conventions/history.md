...
| world | `productivity-snapshots.jsonl`, `productivity-snapshots-*.jsonl` (glob) | The per-iteration composite-score telemetry store and its date segments, added g-358-49 (2026-09-04) when the writer moved off a bare `open(...,"a")` onto `locked_append_jsonl`. This is guard-2415 in its pure form: the locked path calls `save_history()` on **every** append, and the legacy file was 7,417,337 B taking ~662 appends/24h fleet-wide — an unblacklisted cutover would have written ~4.9 GB of `.history` per day, strictly worse than the S3 churn the cutover exists to remove, and invisible until the disk filled. Restore value is nil: append-only telemetry, the file IS the history, changelog still records every write. BOTH names because the writer targets the legacy file when `PRODUCTIVITY_SNAPSHOTS_SEGMENTED` is off and a date segment when it is on; blacklisting one would leave the other snapshotting per append. Glob rather than the strict shape `_productivity_snapshots._SEGMENT_RE` owns, for the same import-cycle reason as the `gate-firings-*` row. Step 3 was a verified no-op: no `.history` subtree existed, because the prior writer was a bare append. |
| agent | `skill-invocations.jsonl`, `health/` | Append-only telemetry ledgers on hot paths, added g-306-118-e when their writers moved off a bare `open(...,"a")` onto `locked_append_jsonl`. Same class as the `gate-firings.jsonl` row: the file IS the history, and they fire once per skill invocation and once per loop iteration respectively, so a full-file snapshot per append is O(N²) over a multi-thousand-line file. (Rows added 2026-09-11 under g-115-9662 — the entries have been in `_SNAPSHOT_BLACKLIST` since g-306-118-e but were never documented here, so this table showed no `agent` key at all.) |
| agent | `capture-evictions-archive.jsonl` | The durable sink `wm-prune` writes each evicted capture entry to before removing it (g-115-9662). Blacklisted for the usual guard-2415 reason — `locked_append_jsonl` calls `save_history()` on **every** append — but the no-restore-value argument is stronger here than anywhere else in this table, and worth stating precisely because the file is itself a RECOVERY layer and so invites the opposite reflex: the sink is append-only and never mutated, so the live file is a strict SUPERSET of every snapshot of it, and restoring one could only ever LOSE archived rows. Step 3 was a verified no-op: `find agents -path '*/.history/*' -name 'capture-evictions-archive*'` returned 0 at add time, because the sink was new in the same change. |
