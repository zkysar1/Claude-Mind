# Partner Liveness — signals, verdicts, and the corroboration protocol

Mechanism reference behind `.claude/rules/check-team-state-before-silent.md`.
The rule keeps the imperatives (probe first; 6h threshold; conclude silence
only on `dormant`; a stale `last_active` is ambiguous and obliges the two-branch
tree). This file carries the WHY and the evidence: what each `liveness-check.sh`
verdict means and where its value came from, the one known fresh-signal false
positive, the two-branch corroboration protocol with its code shape, the
2026-07-14 incident in which the rule's own canonical signal was wrong by 59
hours, and the code consumers wired to the verdict. Loaded on demand
(`load-conventions.sh partner-liveness`); moved out of the rule on 2026-08-17
under g-115-6581 (context-window diet — the rule loaded ~15.6 KB on every turn
of every agent).

## 1. `liveness-check.sh` verdicts and provenance

Preferred probe (multi-signal, cross-box-safe — g-115-2149):
```bash
...
Underlying raw signal (what the helper wraps; use directly only when you just
need the pushed snapshot value, not a liveness verdict):
```bash
bash core/scripts/team-state-read.sh --field agent_status.<partner>.last_active --json
```

## 2. The signal is asymmetric — the one known FRESH false positive (guard-3604)

A **FRESH** `last_active` (within threshold) IS positive evidence of life; that
direction is sound and is the fast path — with ONE known false-positive
generator (guard-3604): a CROSS-AGENT `in_flight` clear (fleet-police duty,
`team-state-clear-in-flight.sh`) bumps the CLEARED agent's
`last_active`, so a freshly-policed DORMANT peer reads `alive` for up
to the full 6h window. Measured 2026-08-13 on echo: `dormant` at age
419min → `alive` at age 1.8min twenty minutes later, `row_updated_by`
naming the clearer, echo not having woken. The bump is DELIBERATE — do
not fix it by removing the stamp (shard merges are whole-snapshot LWW
on `last_active`, `coordination_merge.merge_team_state_shard`, so an
unstamped pop loses the merge and RESURRECTS the cleared claim). It is
also self-concealing: the fresh reading short-circuits liveness-check's
fast path BEFORE any authoritative store read
(`authoritative_last_active_provenance: null` is the tell). So when a
fresh `last_active` follows a known or suspected cross-agent clear — a
coordination-board correction names the peer, or the shard's
`row_updated_by` is not the row's owner — do NOT stop at the fast
path: read a signal with an INDEPENDENT writer from the authoritative
store (`execution-diary.jsonl`, `working-memory.yaml`), per rule 6
Branch 2 and guard-3604 action item 4.

A **STALE** `last_active` is **NOT** evidence of death. It is *ambiguous*,
because a stale value has two causes that look identical from outside: the
partner is idle, **or the partner's heartbeat writer is broken while the partner
keeps working.** The signal cannot distinguish them, and it has been observed
failing this way in production (§4 below). Never conclude silence from a stale
`last_active` ALONE.

## 3. The two-branch decision tree a stale `last_active` obliges

Never conclude silence without walking it. The two branches were found a day
...
## 5. Cross-references and code consumers

- `world/guardrails.jsonl` → `guard-321` (the active guardrail
  enforcing this rule at execution time)
- `world/guardrails.jsonl` → `guard-3604` (rule 5's known false-positive
  generator: a cross-agent `in_flight` clear stamps the cleared peer's
  `last_active`; expect it, post the board correction, never restore the
  old timestamp, and read independent-writer signals inside the window)
