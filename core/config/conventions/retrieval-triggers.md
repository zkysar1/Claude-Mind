...
| G22 | ✓ | **Entry-point honor-system gap** — every other row in this table wires retrieval at the SKILL layer, so all of them share one unstated precondition: that a retrieval-bearing skill fires at all. Nothing guarantees that when a user message arrives. If the model simply answers, retrieval count is zero and **no gate, audit or counter notices** — the absence produces no artifact, which is why this sat undetected while 21 rows above it read as complete coverage. Measured (coach deployment, zc-03, 2026-08-31): reducer log 97 `retrieve.sh` calls, worker 6, assistant session 5 — and all 5 of those were from boot, i.e. **zero** per-message retrievals in the mode the user named as worst. Model-independence is the second half of the argument: a harness can route turns to a different model (measured — zak-code's zakpick routed assistant turns off-Claude), so behavioural discipline tuned on one model does not transfer, while a hook binds every turn regardless of routing. | `core/scripts/user-prompt-retrieval-inject.sh` (UserPromptSubmit, registered in `.claude/settings.json`) runs a read-only `retrieve.sh --depth shallow --read-only --include-framework` pre-pass and injects a ranked INDEX (not an answer) before the model starts thinking. **Structurally fail-open**: exit 2 on UserPromptSubmit *erases the user's prompt*, so every path exits 0 or 1 and an `ERR` trap pins failures to 1. Skips slash/expansion prompts, sentinels, acks, sub-24-char text, and **autonomous sessions** (the loop retrieves for itself — session binding mode wins over the agent-wide file, so an observer beside a RUNNING loop still gets injection). **Latency budget `RETRIEVE_TIMEOUT_S = 5`** — a UX bound, not a correctness one, since blowing it costs one un-injected message; measured 957/1037/1575 ms on cc-08 2026-09-03, and it was 18s until then. **Session dedup** suppresses hits already injected this session (state: `agents/<agent>/sessions/<sid>/prompt-injected-hits.txt`), filtering *before* the per-store cap so a repeat session surfaces the NEXT hits rather than none; when every hit is a repeat it emits a one-line id recap rather than silence, because silence reads to the model as "nothing matched". Dedup fails OPEN toward injecting — a state-file fault must never suppress retrieval. Pins: `core/scripts/tests/test_user_prompt_retrieval_inject.py` (18, mutation-verified). |
