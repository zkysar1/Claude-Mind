...
Domain-specific checks: `core/config/verification-checklist-domain-specific.md` (foundational) + `mind/verification-checklist.md` (agent-discovered). Both read by /verify-learning.
...
1. State machine: `mind/session/agent-state` contains "IDLE" (stopped correctly)
2. Journal entries: `mind/journal/` has session logs with dated entries
3. Aspirations progress: `mind/aspirations.jsonl` — goals attempted, sparks fired
4. Aspirations meta: `mind/aspirations-meta.json` — session_count, readiness_gates
5. Aspirations archive: `mind/aspirations-archive.jsonl` — completed aspirations moved here
6. Evolution log: `mind/evolution-log.jsonl` — evolution events logged
7. Developmental stage: `mind/developmental-stage.yaml` — should be `exploring` (first run)
...
10. Knowledge tree index: `mind/knowledge/tree/_tree.yaml` — nodes registered with `article_count`, `growth_state`, `confidence`, `capability_level`, interior/leaf distinction via `node_type`. All scoring/structural metadata lives exclusively in `_tree.yaml` (split-by-nature schema).
11. No `mind/knowledge/research-queue.yaml` exists (queue eliminated — aspirations handles topic selection)
12. No `mind/knowledge/_index.yaml` exists (index consolidated into `_tree.yaml`)
...
8. `mind/evolution-triggers.yaml` has matching initial state entry for trigger #6
...
11. `mind/evolution-triggers.yaml` has matching initial state entry for `evolution_cadence`
...
10. **Runtime**: If any tree nodes were created or updated, check `mind/knowledge/tree/_tree.yaml` for `entity_index` entries (including at L3+ nodes from DECOMPOSE)
...
4. Knowledge tree: `mind/knowledge/tree/_tree.yaml` — L2+ nodes created if research occurred, L3+ from DECOMPOSE if /tree maintain ran
5. No orphan state: `pipeline-read.sh --counts` stage counts match `pipeline-recompute-meta.sh` recount
6. No stale signals: `mind/session/loop-active` exists if the loop ran; `mind/session/stop-loop` exists if stopped
...
13. `CLAUDE.md` references pipeline JSONL files (not `mind/pipeline/` directories)
14. No `mind/pipeline/` directory references remain in skills: `grep -r "mind/pipeline/" .claude/skills/` returns 0
15. **Runtime**: `mind/pipeline.jsonl` exists (not `mind/pipeline/` directory)
16. **Runtime**: `pipeline-read.sh --counts` stage counts sum matches `wc -l mind/pipeline.jsonl` + archive
...
3. `goal-selector.sh` reads `mind/aspirations.jsonl` via JSONL infrastructure
...
5b. `goal-selector.py` reads epsilon from `mind/developmental-stage.yaml` and noise_scale from `core/config/developmental-stage.yaml`
...
14. **Runtime**: `mind/session/handoff.yaml` contains `first_action` after session-end consolidation
...
17. `core/config/aspirations-initial.jsonl` bootstrap goals use `verification` field
...
30. `aspirations/SKILL.md` Phase 2 reads `mind/session/context-budget.json` for zone
...
36. `core/scripts/context-budget-status.py` reads status line JSON from stdin, writes `mind/session/context-budget.json`
...
### N1. Self File & Rule
1. `mind/self.md` exists with YAML front matter (created, last_updated, last_update_trigger, source)
2. `mind/self.md` has non-empty body content (the Self/Program description)
3. `.claude/rules/self.md` exists — contains directive to consult mind/self.md

### N2. Boot Integration
5. Boot dashboard displays "SELF" section with contents of mind/self.md
6. Boot continuation mode (Step 0.5) reads mind/self.md in abbreviated flow
...
13. `/aspirations evolve` reads mind/self.md before generating aspirations
...
20. `spark-questions-read.sh --id sq-012` returns sq-012 as active with counters (from `mind/spark-questions.jsonl`)
21. `spark-questions-read.sh --candidates` includes sq-c05 (from `mind/spark-questions.jsonl`)
...
### N7. Self Maintenance
25. `/respond` directive table includes "Self/Program update" type
26. sq-012 spark action writes Self evolution proposals to pending-questions.yaml
27. mind/self.md last_update_trigger tracks all update sources
28. Self survives IDLE/RUNNING transitions; wiped only on /reset
...
3. `core/scripts/init-mind.sh` creates `mind/experience/` directory and empty JSONL/JSON files
...
18. **Runtime**: After hypothesis formation, experience `.md` file exists in `mind/experience/`
...
21. **Runtime**: After goal execution, `mind/experience/` has corresponding `.md` file
...
47. `experience-archive.sh` sweeps records matching staleness criteria to `mind/experience-archive.jsonl`
...
### P5. Security
16. No credential values (from `env-read.sh value`) appear in any file under `mind/`
17. No credential values appear in `mind/journal/` entries
18. No credential values appear in `mind/session/handoff.yaml`
...
4. `spark-questions-read.sh --id sq-013` returns sq-013 with status: active (from `mind/spark-questions.jsonl`)
...
14. `core/config/aspirations-initial.jsonl` has two lines: asp-001 (Explore) and asp-002 (Maintain)
15. asp-002 has recurring goals matching `core/config/aspirations-initial.jsonl` with correct `interval_hours`, `priority`, and `recurring: true` fields
16. Each recurring goal has: `recurring: true`, `interval_hours: N`, `lastAchievedAt: null`, full `verification` block. g-002-01 has precondition requiring prior non-recurring goal execution.

### R4. Bootstrap Seeding
18. `core/scripts/init-mind.sh` copies `core/config/aspirations-initial.jsonl` to `mind/aspirations.jsonl` then runs `recompute-all-progress` to derive `total_goals` from actual goal count
19. Boot doesn't duplicate-create aspirations that already exist from init-mind.sh
...
25. **Runtime**: `aspirations-read.sh --active` shows asp-002 with recurring goals
...
30. **Runtime**: g-002-01 is skipped on first boot iteration when no non-recurring goals have been executed
...
5. Base skill files contain no domain-specific terms from `mind/self.md` — verify by grepping key domain nouns from Self against `.claude/skills/*/SKILL.md`
...
8. `core/config/verification-checklist-domain-specific.md` is a template (domain checks live in `mind/verification-checklist.md`)
9. `core/scripts/infra-health.py` has no hardcoded component names or probe functions — components come from `mind/infra-health.yaml`, probes from `mind/core/scripts/probe-{name}.sh`
10. `core/scripts/init-mind.sh` creates `infra-health.yaml` with `components: {}` (empty, not domain-specific)
...
14. `.gitignore` forged skill entries match `mind/forged-skills.yaml` (no orphan entries after reset)
...
_(Domain-specific items live in `mind/verification-checklist.md`, seeded from `core/config/verification-checklist-domain-specific.md` on init)_
...
5. `core/config/aspirations-initial.jsonl` asp-002 includes g-002-05 at HIGH priority with interval_hours: 4
5b. Bootstrap g-002-05 `skill` = `/completion-report` (core framework skill)
5c. Domain-specific wrappers (e.g., email delivery) are forged at runtime and override g-002-05 skill field

### U4. Runtime
11. **Runtime**: `goal-selector.sh` output shows g-002-05 with recurring_urgency raw >= 2.0 when due
12. **Runtime**: g-002-05 scores competitively with HIGH domain goals when due (score >= 7.0)
13. **Runtime**: After 4+ hours of operation, g-002-05 has been selected and executed at least once
...
11. No `mind/knowledge/reasoning-bank/` directory exists (old YAML format deleted)
...
19. No `mind/knowledge/patterns/pattern-signatures.yaml` exists (old YAML format deleted)
...
28. No `mind/spark-questions.yaml` exists (old YAML format deleted)
...
37. No `mind/journal/_index.yaml` exists (old YAML format deleted)
...
41. Output: writes `mind/experiential-index.yaml` (human-readable YAML) + prints JSON to stdout
42. `mind/experiential-index.yaml` lives at top-level mind/ (not mind/knowledge/patterns/)

### V6. Init-Mind Bootstrap
43. `core/scripts/init-mind.sh` creates empty JSONL files: `mind/reasoning-bank.jsonl`, `mind/guardrails.jsonl`, `mind/journal.jsonl`
44. `core/scripts/init-mind.sh` converts spark-questions and pattern-signatures from config YAML initial_state to JSONL via `migrate-yaml` subcommands
45. `core/scripts/init-mind.sh` creates `mind/experiential-index.yaml` at top-level (not patterns/ subdirectory)
46. `core/scripts/init-mind.sh` does NOT create `mind/knowledge/reasoning-bank/` directory (old format)

### V7. Runtime
47. **Runtime**: `mind/reasoning-bank.jsonl` has records after `/reflect` OR Phase 6.5 immediate learning
48. **Runtime**: `mind/guardrails.jsonl` has records after `/reflect` OR Phase 6.5 immediate learning
49. **Runtime**: `mind/pattern-signatures.jsonl` has records with updated accuracy after `/reflect` or `/replay`
50. **Runtime**: `mind/spark-questions.jsonl` shows incrementing `times_asked` counters after goal completions
51. **Runtime**: `mind/journal.jsonl` has one record per session with accumulated goals/events/tags
...
6. CLAUDE.md Internal Skills table lists Prime (called by boot + enterPersona)
...
10. **Runtime**: `/enterPersona` calls `/prime` after setting persona-active to true
11. **Runtime**: Prime loads Self, guardrails, reasoning bank, beliefs, and category-specific tree nodes
12. **Runtime**: Prime output includes PRIMED summary banner with domain counts
13. **Runtime**: `/prime` with UNINITIALIZED state (no agent-state file) outputs "Nothing to prime" and stops
...
7. Phase 6.5 forge awareness registers/increments gaps in `mind/skill-gaps.yaml`, checks forge criteria, creates forge goals via `aspirations-update.sh`
...
5. No file in the repo contains the string `baseline/` (except historical journal entries in `mind/`)
...
### Y3. Forged Skills in mind/
11. `mind/forged-skills.yaml` exists (created by `init-mind.sh` on first boot)
12. Every `.claude/skills/` directory NOT registered in `_tree.yaml` has a matching entry in `mind/forged-skills.yaml`
13. `forge-skill/SKILL.md` writes to `mind/forged-skills.yaml` (NOT `_tree.yaml` or `_triggers.yaml`)
14. `forge-skill/SKILL.md` contains "Do NOT touch `_tree.yaml` or `_triggers.yaml`"
15. `factory-reset.sh` reads `mind/forged-skills.yaml` BEFORE wiping `mind/` to clean forged skill directories
...
### Y5. Factory Reset (Simplified)
21. `factory-reset.sh` does NOT reference `baseline/` or `deploy-baseline.sh`
22. `factory-reset.sh` only does: read forged list → delete forged dirs → `rm -rf mind/`
23. `.claude/` framework files are UNCHANGED after factory reset
24. `/reset` SKILL.md warning text says framework files are unchanged (no "restored from baseline")

---

## Y. Forge-Skill Autonomous Wiring

### Y1. Three Entry Points
...
### Y3. Runtime
13. **Runtime**: After a gap reaches `forge_threshold` encounters, `aspirations-read.sh --active` shows a forge goal with `skill: /forge-skill`
14. **Runtime**: Already-forged gaps (`status: forged` in `mind/skill-gaps.yaml`) do NOT generate new forge goals
15. **Runtime**: `evolution-log-append.sh` entries include `forge-ready` events with source entry point identified
...
6. `mind/forged-skills.yaml` entries use `user` naming, not `owner`
...
6. `mind/session/stop-loop` does NOT exist (stale stop signals must be cleaned up)
...
3. `mind/self.md` contains no-fallback philosophy content _(domain-specific Self content checks moved to `core/config/verification-checklist-domain-specific.md`)_
4. `grep -ri "graceful degradation" mind/knowledge/` returns 0 matches that endorse fallbacks (only matches in negation context like "not graceful degradation" are acceptable)
...
8. `forge-skill/SKILL.md` Step 6 notifies user on validation goal creation using generic intent language
...
12. Worker prompt says "Do READ-ONLY research... Do NOT write files or invoke skills"
...
### AH3. /enterPersona — No Exceptions
21. `enterPersona/SKILL.md` says "User-only control command. Claude MUST NOT invoke this skill." — NO exception text
22. `.claude/rules/user-interaction.md` line 5 has NO team agent exception — clean prohibition
23. `CLAUDE.md` enforcement rule 1 has NO team agent exception — clean prohibition
24. Team agents use `/prime` instead (internal skill, `user-invocable: false`)

### AH4. Tool Access
25. `CLAUDE.md` Tool Access section lists `TeamCreate, SendMessage` alongside existing tools
...
4. `mind/infra-health.yaml` created by `init-mind.sh` with all components initialized to null
...
8. Domain convention `mind/conventions/post-execution.md` Step 1 records infrastructure health after successful infrastructure goals
...
## AN. Hybrid Skill Architecture (completion-report + backlog-report)

Verifies the hybrid skill pattern: user-invocable AND agent-callable. Currently two hybrid skills:
`/completion-report` (core) and `/backlog-report` (core). Domain-specific wrappers (e.g., email delivery) are forged at runtime.

### AN1. Hybrid Skill Category

1. `.claude/skills/completion-report/SKILL.md` has `user-invocable: true` in frontmatter
2. `CLAUDE.md` Skill Invocation Rules has "Hybrid skills" bullet listing `/completion-report` AND `/backlog-report`
3. `CLAUDE.md` User Control Commands table includes `/completion-report` and `/backlog-report` with `ANY` valid-from
...
6. `.claude/skills/_triggers.yaml` has comment noting `/completion-report` is agent-callable
7. Neither `/completion-report` nor `/backlog-report` is in the "MUST NOT invoke" enumerated list (enforcement rule 1)
...
8. `/completion-report` Phase 4 always sets `lastAchievedAt` (report window advances)
9. `/completion-report` Phase 4 sets `status completed` ONLY when called by user directly
10. Domain-specific wrappers (if forged) set `status completed` only after successful delivery

### AN3. Domain Decoupling

11. `/completion-report` contains NO domain-specific script calls, credential checks, or delivery references — pure framework scripts only
12. Domain-specific delivery wrappers live in forged registry, deleted on factory reset

### AN4. Blocked Goals Section

13. `/completion-report` Phase 2 Step 9 calls `goal-selector.sh blocked` (single source of truth — no inline chain-walking)
14. `/completion-report` Phase 3 has "Blocked" section between "Active Work" and "Needs Attention" — omitted when 0 blocked
...
26. `/backlog-report` SKILL.md writes to `mind/BACKLOG.md` (not repo root)
...
30. Phase 5 terminal summary includes absolute path to `mind/BACKLOG.md`
31. `/backlog-report` mutates NO JSONL, NO state files — only writes `mind/BACKLOG.md`
32. `/backlog-report` has NO timer mechanism (unlike `/completion-report` which updates g-002-05)

### AN7. Runtime

33. **Runtime**: User runs `/completion-report` → console report displayed, g-002-05 `lastAchievedAt` updated
34. **Runtime**: `/completion-report` shows "Blocked" section with root bottlenecks when dependency-blocked goals exist
35. **Runtime**: After 4h of RUNNING, g-002-05 selected → `/completion-report` fires (or forged delivery wrapper if exists)
36. **Runtime**: If delivery wrapper fails, g-002-05 is NOT marked completed (retried next cycle)
37. **Runtime**: User runs `/backlog-report` → `mind/BACKLOG.md` written, terminal summary displayed with full file path
...
19. `/completion-report`, `/open-questions`, `/backlog-report` all use `goal-selector.sh blocked` (single source of truth)
...
12. Skills with `conventions: []` (escapePersona, enterPersona, reset) do NOT have Step 0
...
1. `core/scripts/precompact-checkpoint.py` exists — reads working memory, writes `mind/session/compact-checkpoint.yaml`
...
15. Phase -0.5c checks for `mind/session/compact-checkpoint.yaml` existence
...
### AR5. Runtime
26. **Runtime**: After autocompact fires, `mind/session/compact-checkpoint.yaml` exists with correct `compact_count`
...
8. `context-reads.py` scope filter tracks: `core/config/**`, `.claude/skills/**`, `mind/knowledge/tree/**`, `mind/conventions/**`
...
17. All four hooks have `timeout: 5` (seconds)

### AT3. Tracker Lifecycle
18. Tracker file: `mind/session/context-reads.txt` — plain text, first line is `#session:<id>` header, remaining lines are paths
...
21. `context-reads.py invalidate` only removes `mind/knowledge/tree/**` paths (convention files are immutable)
...
29. `_summary.json` is under `mind/knowledge/tree/` — in TRACKED_PREFIXES scope, hooks handle it automatically
...
47. **Runtime**: `mind/session/working-memory.yaml` reads are never tracked (out of scope)
...
6. `mind/knowledge/tree/system.md` has "Loop Integrity" section with goal-selection lesson
7. `mind/knowledge/tree/_tree.yaml` system node summary mentions "loop integrity"
...
4. `self_priorities` are extracted from `mind/self.md` numbered items and `##` headers
...
10. Skipped in UNINITIALIZED state (no mind/ directory)
...
5. **Runtime**: `bash core/scripts/aspirations-update-goal.sh <id> skill /some-skill` stores `/some-skill` (not `C:/Program Files/Git/some-skill`)
6. **Runtime**: Hook scripts (`context-reads-gate.sh`, `context-reads-invalidate.sh`) run without path errors

---

## FS. Forged Skills Gitignore

1. Every entry in `mind/forged-skills.yaml` has a matching `.claude/skills/{name}/` line in `.gitignore`
...
10. `mind/session/handoff.yaml` exists after /stop (Step 9 ran)
11. `mind/session/working-memory.yaml` is reset to template state (Steps 4-5 ran)
...
5. `settings.json` `permissions.deny` does NOT contain `*/core/scripts/*` pattern — collides with writable `mind/core/scripts/`
6. `settings.json` `permissions.deny` includes `Edit(*/core/config/*)`, `Edit(*/.claude/rules/*)`, `Edit(*/CLAUDE.md)`
7. `settings.json` `permissions.deny` includes `Edit(*/mind/knowledge/tree/_tree.yaml)` and `Edit(*/mind/*.jsonl)` — forces script access
...
3. `wm-init.sh` creates `mind/session/working-memory.yaml` with 15 slots + `slot_meta` section
...
15. Grep `.claude/skills/` for `Read mind/session/working-memory` returns 0 matches
16. Grep `.claude/skills/` for `Write mind/session/working-memory` returns 0 matches
17. Grep `.claude/skills/` for `Edit mind/session/working-memory` returns 0 matches
...
19. `mind/knowledge/archive/` directory exists for distilled/retired node content
