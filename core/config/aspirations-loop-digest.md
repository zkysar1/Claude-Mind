...
### LLM-owned fields (still serialize in LOOP_CONTINUE)

`evolutions_this_session`, `last_evolution_goal_count`,
`goals_since_last_alignment_check`, `aspirations_touched_this_session`,
`signals.consecutive_goal_failures`, `signals.last_failed_goal_id`,
`idle_fallback_created` (set by aspirations-all-blocked B2.5).

### Read-merge-write at LOOP_CONTINUE

To avoid clobbering bash mutations, the LLM MUST read loop_state fresh from WM
at LOOP_CONTINUE prep, overlay only LLM-owned fields onto the loaded dict, and
write back. NEVER write loop_state from a stale snapshot held since the start
of the iteration — bash gates have run since.
...
```
  ...
  Phase 4.    Bash: execution-diary.sh phase-start phase-4-execute --goal {goal.id}
              Claim-conflict gate (live partner snapshot — see coordination.md
              ...
              and LOOP_CONTINUE.
              IF source==world: aspirations-claim.sh (conflict → phase-end + LOOP_CONTINUE).
...
LOOP_CONTINUE:
  # g-283-04/05/06 closed: LLM-side wm-set loop_state mirror fully retired.
  # Bash gates own ALL loop_state writes: recurring-loop-state-mutate.py
  # (routine_streaks, signals, *_this_session counters via recurring-close),
  # loop-state-bump-counters.py (goals_completed, productive_goals via
  # iteration-close do_state_update for both recurring + non-recurring).
  # Pinned by test_compact_restore_loop_state_shape.py (g-283-03) and
...
2. `loop_state` is maintained by bash gates only (no LLM-side mirror).
   The bash gates write ALL iteration state variables including
   `routine_streaks`. If a counter silently corrupts, that's a bash-gate
   bug to fix in the writer, not by reviving the LLM mirror.
