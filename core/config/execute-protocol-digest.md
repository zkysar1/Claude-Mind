...
# Tier 2.5 (routine since 2026-08-21): still insufficient after tree+codebase,
# or the subject may be owned by another deployment →
#   Bash: peer-retrieve.sh --json "{topic}"
# One cheap read-only call BEFORE any web search. Read the rc: only `empty` at
# `completeness: complete` (rc=0) licenses a negative; rc=3/partial means a
# lane was BLIND (retrieval-escalation.md Tier 2.5).
