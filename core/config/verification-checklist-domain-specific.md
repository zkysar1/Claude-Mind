...
Check: the bound agent's `self.md` is non-empty and was updated within the last 90 days. Bash: `py -3 -c "import os,sys,pathlib,time; agent=os.environ.get('MIND_AGENT'); print('N/A: MIND_AGENT not set — skipping per-agent self.md check') or sys.exit(0) if not agent else None; p=pathlib.Path(agent)/'self.md'; ok=p.exists() and p.stat().st_size>0; age_d=(time.time()-p.stat().st_mtime)/86400 if p.exists() else 999; print(f'PASS: {agent}/self.md present ({age_d:.0f}d since update)') if ok and age_d<=90 else (print(f'WARN: {agent}/self.md missing or {age_d:.0f}d stale (>90d) — agent identity may have drifted') if ok else print(f'FAIL: {agent}/self.md missing or empty'))"`
