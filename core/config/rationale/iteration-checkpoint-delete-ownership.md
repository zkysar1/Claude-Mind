...
`iteration-close.sh:3687` runs `rm -f "$AGENT_DIR/session/iteration-checkpoint.json"`.
...
|---|---|---|
| `iteration-close.sh:2457` | `selected_at` | tree-updated validation when `--tree-updated` was NOT passed |
| `iteration-close.sh:2483` | `selected_at` | tree-updated VALIDATION when `--tree-updated` WAS passed |
| `iteration-close.sh:2551` | `selected_at` | metric-encoding probe → `force_metric_encoding_pending` |
...
rm in the loop digest" that `iteration-close.sh:3687`'s comment names. Checked
...
discriminator is not the `rm` — it is what runs between it and `:3687`.
...
- `core/scripts/iteration-close.sh` `:2457` / `:2483` / `:2551` (readers),
  `:3687` (the owning deleter)
