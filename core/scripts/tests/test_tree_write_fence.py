"""Two-writer regression tests for tree_write_fence (g-115-3555).
...
# ------------------------------------------------- shell-wrapper wiring (g-115-3720)
...
#
# Root cause: AYOAI_AGENT is injected only into PreToolUse[Bash], NOT into
...
def _run_hook(op, file_path, agent_dir, session_id="", agent="alpha"):
    ...
    env = dict(os.environ)
    env["AYOAI_AGENT_DIR"] = str(agent_dir)   # _paths.py test override -> hermetic
...
def test_wrapper_does_not_depend_on_ayoai_agent_being_injected():
    ...
    assert "session-binding-read.sh" in code, (
        ...
        "again (AYOAI_AGENT is not injected outside PreToolUse[Bash])")
