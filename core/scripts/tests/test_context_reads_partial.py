"""g-115-3747: ranged (offset/limit/pages) reads must reach the manifest.
...
# ADVISORY-ONLY scope (core/scripts, g-115-2210): recorded + advisory, never blocked.
...
def _throwaway_agent(manifest_paths=None, partial_paths=None, session_id=SID):
    ...
    try:
        ...
        env = dict(os.environ)
        env["AYOAI_AGENT"] = THROWAWAY_AGENT
        yield env
    finally:
