...
def test_this_file_never_mocks_the_reader():
    ...
    Matches INJECTION SHAPES (`setattr` / `patch(`), not the bare word "mock".
    The first draft also matched "mock" anywhere on the line and immediately
    failed on this file's own fixture docstring ("No _load_team_state_cached
    mock.") — a checker whose input assumption was wrong, flagging prose as
    code, which is the exact `checker-input-assumption-defects` shape this whole
    file exists to guard against. Kept as a comment because a future "simplify"
    pass would otherwise re-widen it.
    """
    src = Path(__file__).read_text(encoding="utf-8")
    offenders = [
        ln for ln in src.splitlines()
        if "_load_team_state_cached" in ln
        and ("setattr" in ln or "patch(" in ln)
    ]
