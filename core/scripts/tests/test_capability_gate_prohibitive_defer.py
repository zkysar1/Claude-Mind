"""test_capability_gate_prohibitive_defer.py — regression tests for .
...
def test_prohibitive_defer_emits_no_unblock_verb():
    """The canonical  /  shape must suppress the Unblock.
...
def test_create_blocker_context_is_byte_compatible(ctx):
    """Default and explicit create-blocker keep the pre- wording.
...
# below survived  undetected: the fix was applied to
...
# can redirect and explicitly does NOT honour on this path ().
# Measured 2026-07-29 on : following the gate's own message verbatim
...
def test_daemon_defer_path_recommends_the_flag_it_honours():
    ...
    assert "--force-defer" in reason, (
        ...
        f"actually reads (X-Mind-Force-Defer, per aspirations_write.py). "
