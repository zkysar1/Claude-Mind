""" / -A5: unblock-parent-status-sweep.py corruption-tolerant
parse for _read_aspirations (line 145 in audit; now via _tolerant_decode).

Paired with g-115-797 (bravo audit catalog) row A5. Before this fix, the
helper `_read_aspirations` did `json.loads(raw)` then `except
json.JSONDecodeError: return []` — which silently collapsed the aspiration
list to 0 under the g-115-766 daemon aggregate-corruption mode (valid JSON
prefix + trailing garbage written during a stale-code reload window).
Downstream effect: 0 Unblock-titled goals seen, no parent-status sweep
matches fire — silently breaking the Layer D parent-resolution guard.

The fix applies the consolidation-health.py::_tolerant_decode pattern
identical to the A4 sibling (parent-supersession-sweep), with two
adaptations:
  1. Accepts BOTH dict-with-aspirations-key AND bare-list aggregates (the
     daemon may emit either; the existing call site at line 211 already
     extracts via `data.get("aspirations") if isinstance(data, dict) else data`)
  2. Empty body returns None (not []) — the caller maps None -> [] so it
     can distinguish "empty queue" (valid) from "decoded as a dict"
     (extract via "aspirations" key)

guard-383 (N>=2-source aggregator pattern): `_read_aspirations` is called
for BOTH world and agent sources at line 263-264 and concatenated. A
per-source error must be FATAL (sys.exit(1)) — never silent [] — to avoid
poisoning the merged aggregate with a complete-looking lie.
...
Run: py -3 -m pytest core/scripts/tests/test_unblock_parent_status_sweep_tolerant_parse.py -v
"""
import contextlib
...
def load_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "unblock_parent_status_sweep_module",
...
def test_valid_populated_list():
    payload = '[{"id": "asp-115", "status": "active"}, {"id": "asp-001"}]'
...
def test_valid_dict_with_aspirations_key():
    # Adaptation from consolidation-health: unblock-parent-status-sweep
    # handles BOTH dict-with-aspirations and bare-list aggregates.
    payload = '{"aspirations": [{"id": "asp-115"}, {"id": "asp-001"}], "meta": {}}'
    result, err = _decode("world", payload)
...
def test_unrecoverable_body_is_fatal_with_one_diagnostic():
    ...
    assert lines[0].startswith("unblock-parent-status-sweep: agent "), (
        f"diagnostic must carry the symmetric 'unblock-parent-status-sweep: "
...
def test_read_aspirations_rterror_is_fatal_with_loud_diagnostic():
    ...
    try:
        ...
        assert e.code == 1, f"expected SystemExit(1), got SystemExit({e.code!r})"
        err = buf.getvalue()
        assert "unblock-parent-status-sweep: world read failed" in err, (
