...
export INFRA_STREAK_BOARD_STUB="$TMPDIR_T/default-board-stub.jsonl"
...
set +e
out=$(SHIM_MODE=crash PATH="$SHIM_DIR:$PATH" bash "$WRAPPER" 2>&1)
rc=$?
set -e
...
set +e
out=$(SHIM_MODE=designed PATH="$SHIM_DIR:$PATH" bash "$WRAPPER" 2>&1)
rc=$?
set -e
...
{"components": [{"component": "ci", "consecutive_failures": 16, "last_failure": "2026-07-18T18:34:04",
  ...
  "human_gated": false, "notify_suppressed_agents": []}]}
EOF
BOARD_STUB="$TMPDIR_T/board-stub.jsonl"
: > "$BOARD_STUB"
