...
# pattern-signatures-update-field -- daemon-aware wrapper. Updates a single
# field on a pattern-signature record.
#
# Usage:  pattern-signatures-update-field.sh <id> <field> <value>
#
set -euo pipefail
...
argv_strict_parse "pattern-signatures-update-field.sh" 3 "$@"
REC_ID="${ARGV_POS[0]:-}"
FIELD="${ARGV_POS[1]:-}"
VALUE="$(argv_strict_resolve_value "pattern-signatures-update-field.sh" "${ARGV_POS[2]:-}")"

if [ -z "$REC_ID" ] || [ -z "$FIELD" ] || [ -z "$VALUE" ]; then
    argv_strict_usage "pattern-signatures-update-field.sh" 3
    exit 1
fi
...
QUERY="store=pattern-signatures&id=$(rt_url_encode "$REC_ID")&field=$(rt_url_encode "$FIELD")&value=$(rt_url_encode "$VALUE")"
...
case $rc in
    ...
    3)
        ...
        rt_no_daemon_error "pattern-signatures-update-field.sh";;
    *) exit $rc;;
esac
