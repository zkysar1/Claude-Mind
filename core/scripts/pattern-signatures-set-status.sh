...
# pattern-signatures-set-status -- daemon-aware wrapper. Updates the status
# field on a pattern-signature record (set-field specialization).
#
# Usage:  pattern-signatures-set-status.sh <id> <status>
#
set -euo pipefail
...
if [ -z "$REC_ID" ] || [ -z "$STATUS" ]; then
    echo "Usage: pattern-signatures-set-status.sh <id> <status>" >&2
    exit 1
fi
...
QUERY="store=pattern-signatures&id=$(rt_url_encode "$REC_ID")&field=status&value=$(rt_url_encode "$STATUS")"
...
case $rc in
    ...
    3)
        ...
        rt_no_daemon_error "pattern-signatures-set-status.sh";;
    *) exit $rc;;
esac
