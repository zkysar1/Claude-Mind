...
# journal-update — daemon-aware wrapper. Full-replace a journal session
# index record.
#
# Usage:  journal-update.sh <session-id>   (session-N or bare N) + stdin JSON
#
set -euo pipefail
...
if [ -z "$SESSION_ID" ]; then
    echo "Error: journal-update.sh requires a session id argument (session-N or N)." >&2
    exit 1
fi
...
case $rc in
    ...
    3)
        ...
        rt_no_daemon_error "journal-update.sh";;
    *) exit $rc;;
esac
