...
# guardrails-add — daemon-aware wrapper. Appends a guardrails record
# from stdin JSON.
#
# Hot path:
#   1. Skinny PROJECT_ROOT resolve (no _paths.sh)
#   2. Read stdin body (JSON guardrails record)
#   3. POST /v1/store/append?store=guardrails
...
if [ "$SCHEMA" = "1" ]; then
    echo "Error: --schema is no longer available. See mind_api/src/endpoints/store.py + mind_api/src/store_registry.py for the guardrails record contract." >&2
    exit 1
fi

# Read stdin (the JSON guardrails record) BEFORE invoking the daemon.
BODY="$(cat)"
...
case $rc in
    ...
    3)
        ...
        rt_no_daemon_error "guardrails-add.sh";;
    *) exit $rc;;
esac
