...
# bash-builtin only, fail-open. mtime of core/logs/hook-fires/session-manifest-write-gate-hook
# = last fire of this hook.
{ _HF_DIR="${BASH_SOURCE[0]%/*}/../.." ; mkdir -p "$_HF_DIR/core/logs/hook-fires" 2>/dev/null && : > "$_HF_DIR/core/logs/hook-fires/session-manifest-write-gate-hook" 2>/dev/null ; unset _HF_DIR ; } 2>/dev/null || true

# PreToolUse[Write|Edit|MultiEdit] hook adapter — session-manifest-write-gate.
#
# Thin bash wrapper. The Python body lives in
# session-manifest-write-gate-hook.py because a heredoc on `python -` would
# consume stdin before json.load(sys.stdin) runs.
#
...
SCRIPT_PATH="$PROJECT_ROOT/core/scripts/session-manifest-write-gate-hook.py"
