#!/usr/bin/env bash
# Scan world+agent queues for aged goals carrying `user` in participants and
# (optionally) EMAIL THE USER, plus post a coordination-board record that
# doubles as the shared cooldown. See user-blocker-escalation-check.py for the
# full docstring + design rationale.
#
...
#
# Usage: user-blocker-escalation-check.sh [--apply] [--escalate-hours N] [--agent NAME]
