...
# Concurrent agents use AYOAI_AGENT env prefix instead.
if not os.environ.get("AYOAI_SESSION_ID"):
    _sid_file = PROJECT_ROOT / ".latest-session-id"
    if _sid_file.exists():
        os.environ.setdefault("AYOAI_SESSION_ID",
                              _sid_file.read_text(encoding="utf-8").strip())
...
def _resolve_agent_name():
    """Resolve agent name: AYOAI_AGENT env > .active-agent-$SESSION_ID > .active-agent global.
    If AYOAI_AGENT is explicitly set (even to empty), it takes priority.
    Session-keyed file prevents cross-agent contamination when multiple agents
    run in separate Claude Code windows."""
    if "AYOAI_AGENT" in os.environ:
        return os.environ["AYOAI_AGENT"].strip()
    sid = os.environ.get("AYOAI_SESSION_ID", "")
    if sid:
        sf = PROJECT_ROOT / f".active-agent-{sid}"
        if sf.exists():
...
# Resolution: AYOAI_AGENT env > .active-agent-$SESSION_ID > .active-agent global
# (see _resolve_agent_name above — keep in sync)
