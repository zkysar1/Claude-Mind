...
def assert_world_dir(module_name: str = "<unknown>") -> None:
    ...
    if WORLD_DIR is None:
        msg = (
            ...
            f"WORLD_DIR to resolve to a real external path. If this is raised "
            f"inside a LONG-RUNNING DAEMON (mind_api) the config on disk may be "
            f"correct and the daemon stale — it bound WORLD_DIR at import, before "
            f"this world was configured; restart it (mind-api-start.sh --restart) "
            f"or pass the request's world root explicitly (g-367-08). Otherwise, "
            f"typical fix: bind "
