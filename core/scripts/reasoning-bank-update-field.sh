...
# reasoning-bank-update-field — daemon-aware wrapper. Updates a single
# field on a reasoning-bank record.
#
# Usage:  reasoning-bank-update-field.sh <id> <field> <value>
#
set -euo pipefail
...
QUERY="store=reasoning-bank&id=$(rt_url_encode "$REC_ID")&field=$(rt_url_encode "$FIELD")&value=$(rt_url_encode "$VALUE")"
...
case $rc in
    ...
    3)
        ...
        rt_no_daemon_error "reasoning-bank-update-field.sh";;
    *) exit $rc;;
esac
