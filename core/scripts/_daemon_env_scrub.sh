...
daemon_scrub_inherited_env() {
    ...
    for _v in $(compgen -e); do
        case "$_v" in
            ...
            MIND_ALLOW_TMP_OWNCLOUD_PUT|STORAGE_BACKEND|STORAGE_S3_BUCKET|STORAGE_S3_ENDPOINT_URL|STORAGE_DDB_SESSIONS_TABLE|STORAGE_DDB_LOCK_TABLE|ENVIRONMENT_ID|MACHINE_ID|MACHINE_MULTI|OWNCLOUD_SYNC_INTERVAL|OWNCLOUD_CACHE_TTL|MIND_API_TOKEN|MIND_API_BIND|MIND_API_PORT) unset "$_v" ;;
        esac
    done
}
