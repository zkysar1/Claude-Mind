...
# bash-builtin only, fail-open. mtime of core/logs/hook-fires/mutation-proof-hand-roll-gate
# = last fire of this hook.
{ _HF_DIR="${BASH_SOURCE[0]%/*}/../.." ; mkdir -p "$_HF_DIR/core/logs/hook-fires" 2>/dev/null && : > "$_HF_DIR/core/logs/hook-fires/mutation-proof-hand-roll-gate" 2>/dev/null ; unset _HF_DIR ; } 2>/dev/null || true

# PreToolUse[Bash] hook — ADVISORY for the hand-rolled mutation-proof shape
# (, closing a four-artifact honor-system cluster at the tool layer
# after four recurrences across three agents).
#
# Thin bash wrapper. The Python body lives in
# mutation-proof-hand-roll-gate.py because a heredoc on `python -` would
...
SCRIPT_PATH="$PROJECT_ROOT/core/scripts/mutation-proof-hand-roll-gate.py"
