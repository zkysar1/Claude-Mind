...
# Resolve the agent. MIND_AGENT is NOT injected into Read/Write/Edit hooks --
...
# never fire. Measured 2026-07-28 (): 18h live, zero baselines, zero
...
# No `2>>/dev/stderr` here (removed ). It was a no-op by intent —
# stderr already passes through when only stdout is redirected — but on Git Bash
# /dev/stderr does not resolve when stderr is a pipe rather than a tty, so the
# redirect itself failed with "No such file or directory" and `|| exit 0` ate
# the whole invocation. The fence therefore never ran on Windows under any
