...
# pattern-signatures-add -- daemon-aware wrapper. Appends a pattern-signature
# record from stdin JSON.
#
# Hot path:
#   1. Skinny PROJECT_ROOT resolve (no _paths.sh)
#   2. Read stdin body (JSON pattern-signature record)
#   3. POST /v1/store/append?store=pattern-signatures
...
if [ "$SCHEMA" = "1" ]; then
    echo "Error: --schema is no longer available. See mind_api/src/endpoints/store.py + mind_api/src/store_registry.py for the pattern-signatures record contract." >&2
    exit 1
fi

# Read stdin (the JSON pattern-signature record) BEFORE invoking the daemon.
BODY="$(cat)"
...
RESPONSE="$(rt_call POST /v1/store/append \
    --query "store=pattern-signatures" \
    --body-string "$BODY")" || rc=$?
...
case $rc in
    ...
    3)
        ...
        rt_no_daemon_error "pattern-signatures-add.sh";;
    *) exit $rc;;
esac
