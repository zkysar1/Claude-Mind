"""End-to-end wrapper test for aspirations-recover-recurring.sh (PR 53).
...
WRAPPER = REPO_ROOT / "core" / "scripts" / "aspirations-recover-recurring.sh"
...
def test_wrapper_happy_path(running_daemon):
    """Daemon path: recovers corrupted goal, exits 0."""
...
def test_wrapper_no_recovery(running_daemon):
    """Nothing to recover → recovered=0."""
    project_root, _ = running_daemon
    world = project_root / "world"
    _write_jsonl(world / "aspirations.jsonl", [
        _make_asp("asp-001", "active", goals=[
