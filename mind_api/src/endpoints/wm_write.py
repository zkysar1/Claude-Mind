...
def append_slot(ctx) -> "Response":  # type: ignore[name-defined]
    ...
    _carrier_path = None
    #  F6: THREE-VALUED on purpose, and None is a distinct fact — "no
    # carrier write was attempted for this append" (not a capture slot, or the
    # entry is not load_bearing). Collapsing it into False would make "nothing
    # to deliver" byte-indistinguishable from "delivery failed", which is the
    # always-reports-clear defect this field exists to remove, pointed the other
    # way. Same shape as verify-check-eval's all_passed:null (guard-1760) and as
    # this wrapper's own absent-placement branch, which prints nothing because
    # "the resolver never ran" is a third fact rather than a placement.
    ...
    if _carrier_path is not None:
        _cm = _carrier_mod(ctx)
        if _cm is not None:
            # ASSIGN, never a bare statement (guard-5324): push() returns a bool
            # by contract and swallows its own transport failure, so a discarded
            # return made a total delivery failure byte-indistinguishable from
            # total success. The carrier's one failure message is emitted ONCE
            # PER PROCESS to stderr, which on this path is the DAEMON's log, not
            # the caller's terminal — so for a caller the absence of that line
            # was the only available evidence, and rb-5828 says a green check
            # that cannot be shown to go red is not evidence at all.
            # WHAT True MEANS, precisely, because overclaiming here would ship a
            # new always-clear signal into the goal that exists to kill them: the
            # backend write RETURNED WITHOUT RAISING. It is an ATTEMPT, not a
            # verified delivery — this module's own docstring records a carrier
            # holding 151,390 local bytes whose S3 key was ABSENT, and only an
            # authoritative read-back can settle presence.
            _carrier_pushed = bool(_cm.push(_carrier_path))
    ...
    #  F6: `carrier_pushed` is always present for the same reason
    # `evicted` and `placement` are — a caller must be able to branch on it
    # without a key-existence check, and the write that CAUSED the push is the
