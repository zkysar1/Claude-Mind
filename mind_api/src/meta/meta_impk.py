...
# MIRROR of core/scripts/meta-impk.py SIGNIFICANCE_SE_MULTIPLE. This copy is the
# LIVE one: meta-impk.sh is a daemon-only wrapper, so the CLI file alone changes
# nothing at runtime (guard-742/547). Edit BOTH or neither.
...
def _significance_param(ctx):
    """Optional ?significance=<float> override; falls back to the module default.

    Fail-soft on a malformed value rather than 400: this parameter only tunes
    label sensitivity, and refusing the whole computation over it would take a
    working metric offline for a cosmetic input error.
    """
    raw = ctx.query.get("significance")
    if raw is None or raw == "":
        return SIGNIFICANCE_SE_MULTIPLE
    try:
        return float(raw)
...
def compute(ctx) -> "Response":  # type: ignore[name-defined]
    ...
    # falling to "stable". No uncovered gap.
        if older:
            ...
            # See the CLI mirror: absent a baseline the old code reported
            # "improving" on no evidence. "stable" + null significance is the
            # honest verdict and the one that does not fire the META ALERT.
            recent_avg = sum(e.get("learning_value", 0) for e in recent) / len(recent)
            delta = None
